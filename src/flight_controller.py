...
class FlightController:
    def __init__(self, serial_port="/dev/ttyACM2", baudrate=115200):
        ...
        self.board = MSPy(device=self.serial_port, loglevel='DEBUG', baudrate=self.baudrate)
    ...
    def read_imu(self):
        ...
        try:
            if self.board.send_RAW_msg(MSPy.MSPCodes['MSP_RAW_IMU']):
                ...
                self.board.process_recv_data(dataHandler)  # Ensures SENSOR_DATA is updated

                # Retrieve raw IMU data
                raw_accel = self.board.SENSOR_DATA['accelerometer']
                raw_gyro = self.board.SENSOR_DATA['gyroscope']
                raw_mag = self.board.SENSOR_DATA['magnetometer']
...
while True:
    try:
        ...
        # Send RC command as needed, e.g., [1500, 1500, 1500, 1500, 1500, 1500, 1500, 1500]
        # fc.send_rc_command([1500, 1500, 1500, 1500, 1500, 1500, 1500, 1500])
